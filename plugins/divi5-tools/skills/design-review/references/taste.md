...
| **MOTION** | static, hover only | scroll reveal, sticky | Divi animation presets + sticky options — **keep restrained**; Divi motion is limited, half-built motion is worse than none. **Optional:** when DiviTheatre is installed (user-confirmed), map this dial to real Theatre.js presets via the landing-page skill's divi-theatre.md |
...
Anti-slop rules alone do not produce Floria-tier pages. Before building or reviewing, read
**`skills/landing-page/references/layout-patterns.md`** and study the Floria reference screenshots
(`floria-top.png`, `floria-bottom.png` in that folder).
...
- **Image-led sections** where the brief has products, portfolio, or lifestyle content.
- **Varied section headers** — left-aligned for split/gallery sections; centred only for manifesto bands.

If the layout could be mistaken for a default Divi demo (centred hero, three equal blurbs, eyebrow
